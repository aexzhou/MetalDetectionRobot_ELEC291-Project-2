/*	
	...
	- Initialization functions (c51_external_startup, ADC, UART)
	- ADC reading functions
	- UART handling
	- Transmission Code
	- Timer Code
		-Timers Used: Timer0, Timer3, Timer4
...
#define TIMER_4_FREQ 1000 //timer 4 interrupts at 1000 hz
...
char _c51_external_startup (void)
{
	...
	P3MDOUT |= 0x08; // P2.1 in push-pull mode
...
void InitPinADC (unsigned char portno, unsigned char pin_num)
{
	...
	SFRPAGE = 0x00;
	printf("Initialized ADC at Pin %d_%d\n",portno,pin_num);
}
...
{
	printf("Command: %s", s);
	P3_3=0; // 'set' pin to 0 is 'AT' mode.
	waitms(5);
	sendstr1(s);
	getstr1(buff);
	waitms(10);
	P3_3=1; // 'set' pin to 1 is normal operation mode.
	printf("Response: %s\r\n", buff);
}

char Package_Signed_int(int in){
	char out = 0x80; //set bit 7, 6 = 0b10 to indicate motor cmd
	
	if(in<0){
		out |= 0x20; //set bit 5 to 1 if negative
	}
	out |= abs(in) & 0x1F; //preserve only bits 0-4
	return out;
}
